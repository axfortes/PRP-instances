...
std::vector<int> prts (7,0); //perturbations
std::vector<int> sini (5,0); //initial VRP solutions

//parametros calibrados pelo irace, config 19
int maxIter = 300;  //numero maximo de iterações do laco principal
int maxPertKK = 15; //numero de perturbacoes do tipo Swap(K,K)
int maxPertRD = 3;  //numero de perturbacoes do tipo Randomica
int maxPertLX = 7;  //numero de perturbacoes do tipo Lexicografica
int maxPertP = 3;   //numero de perturbacoes do tipo Plano de Producao
int maxWeight = 5;  //peso maximo adotado na perturbacao dos delta
double alpha = 0.2; //distancia máxima da solucao corrente
...
route reverseRoute(data d, route r0, int v);
bool splitRoute(data d, int t, routing &r, vrs &s);
bool kkSwapRoutes(data d, vrs &s, routing &r, int t);
bool lexicographic(data d, std::vector<int> q, vrs &s);
bool randomized(data d, vrs &s);
bool perturbations(data d, int t, int pT, routing &r, std::vector<vrs> &s);

//production perturbations
...
void read(data &d){
    ...
    d.ntMax = 5 + 1;    //5 de roteamento e 1 de producao
...
void updateVRS(data d, vrs &s, routing &r, double bestDiff, int bV1, int bV2, std::deque<int> bR1, std::deque<int> bR2, int t){    
    //atualizando solução            
    s.c += bestDiff;
    ...
    //atualizando a rota que perdeu os clientes    
    if (bR1.size() > 0){
...
bool shift(data d, vrs &s, routing &r, int t, int K){
    ...
    do {
        ...
        if (melhora){
            updateVRS(d,s,r,bestDiff,bV1,bV2,bR1,bR2,t);
            /*
            //atualizando solução
...
//routing perturbations
route reverseRoute(data d, route r0, int v){
    //std::cout << green << "\n\t\tReverse route" << normal;
    std::deque<int> rota;
    //printRoute(d,r0,v);
    for (int pos = 0; pos < r0.rOrder.size(); pos++) rota.push_front(r0.rOrder[pos]);
    route r1 = newRoute(r0.rCost, r0.rTTT, r0.rLoad, rota, r0.rComp);
    //printRoute(d,r1,v);
    //std::cout << green << "\n\t\tfim reverse" << normal;
    return r1;
}
//so chamo essa perturbacao if (r.V[t] > 0 && r.V[t] < d.V)
bool splitRoute(data d, int t, routing &r, vrs &s){
    //std::cout << green << "\n\t\tSplit route" << normal;
    bool splitted = false;
    //vs eh o veiculo atualmente nao utilizado
    int vs = 0;
    for (int v = d.V; v >= 1 && vs == 0; v--) if (!r.g[t][v]) vs = v;

    //se existe veiculo vazio, transfiro para ele, senao retorno falso
    if (vs == 0) return false;
    //else std::cout << "\n\t\tv = " << vs;

    std::uniform_int_distribution<> V(1,d.V);
    int v = V(gen);
    while (v == vs || !r.g[t][v]) {
        v = V(gen);
        //std::cout << "\n4 v = " << v;
    ...
    boost::dynamic_bitset<> N1(d.n,0);
    boost::dynamic_bitset<> N2(s.R[v].rComp);
    std::deque<int> r1;
    std::deque<int> r2 (s.R[v].rOrder);
    int load = 0;
    for (int p = 0; p < s.R[v].rOrder.size(); p++){
        int i = s.R[v].rOrder[p];
        if (load <= d.Q[vs]) {
            load += r.q[t][i];
            N1.set(i-1);
            N2.reset(i-1);
    ...
    vrs sl = cpyVRS(s);
    if (N1.any()){
        sl.R[vs] = newRoute(d.e[vs] + checkRoutCost(d,r1), checkRoutTTT(d,r1), load, r1, N1);
        /*
        std::cout << blue;
        printRoute(d,sl.R[vs],vs);
        std::cout << normal;
        //*/
        //atualizando s
        sl.c = sl.c - s.R[v].rCost + sl.R[vs].rCost;
        for (int pos = 0; pos < sl.R[vs].rOrder.size(); pos++) sl.cpp[sl.R[vs].rOrder[pos]] = vs;
        if (sl.R[vs].rOrder.size() > 0) updatePredSucc(d.n, sl.R[vs].rOrder, sl.ps);

        //se a rota v tem algum cliente ainda
        if (N2.any()){
            sl.R[v] = newRoute(d.e[v] + checkRoutCost(d,r2), checkRoutTTT(d,r2), d.Q[v] - load, r2, N2);

            //atualizando s
            sl.c = sl.c + sl.R[v].rCost;
            for (int pos = 0; pos < sl.R[v].rOrder.size(); pos++) sl.cpp[sl.R[v].rOrder[pos]] = v;
            if (sl.R[v].rOrder.size() > 0) updatePredSucc(d.n, sl.R[v].rOrder, sl.ps);

            //s = cpyVRS(sl);
            //atualizar r
            r.g[t][vs] = true;
            r.z[t][vs] = boost::dynamic_bitset<> (sl.R[vs].rComp);
            r.z[t][v] = boost::dynamic_bitset<> (sl.R[v].rComp);
            /*
            std::cout << green;
            printRoute(d,sl.R[v],v);
            std::cout << normal;
            //*/
            splitted = true;
        }else{
            ...
            r.g[t][vs] = true;
            r.z[t][vs] = boost::dynamic_bitset<> (s.R[vs].rComp);
            r.V[t]++;
            splitted = true;
        }
    }else
    ...
    if (splitted) s = cpyVRS(sl);

    //std::cout << green << "\n\t\tfim split" << normal;
    return splitted;
}
//pega strings de clientes de rotas diferentes e troca eles, similar a CROSS
bool kkSwapRoutes(data d, vrs &s, routing &r, int t){
    //std::cout << "\n\tSwap KK";
    //sinaliza perturbacao na solucao
    bool trocou = false;

    int maxRouteLenght = 0;
    for (int v = 1; v <= d.V; v++) maxRouteLenght = std::max(maxRouteLenght,int(s.R[v].rOrder.size()));

    if (maxRouteLenght == 0) return false;
    //sorteando o tamanho dos strings
    std::uniform_int_distribution<> K(1,maxRouteLenght);
    std::uniform_int_distribution<> V(1,d.V);

    //sinaliza se a rota jah foi perturbada, se jah, ela nao sera mais. Isso evita que movimentos sejam desfeitos
    std::vector<bool> mudou (d.V + 1,false);

    bool perturbou = false;
    //int maxPert = d.V * 5;
    int totPert = 0;
    do{
        int K1 = K(gen);
        ...
        int v1 = V(gen);
        while(s.R[v1].rOrder.size() == 0) v1 = V(gen);
        while (s.R[v1].rOrder.size() < K1){
            //v1 = V(gen);
            K1 = K(gen);
            //std::cout << "\n\tv1 = " << K1 << " >> sz = " << s.R[v1].rOrder.size();
        }

        int v2 = V(gen);
        while (v2 == v1 || s.R[v1].rOrder.size() == 0){
            v2 = V(gen);
            while (s.R[v2].rOrder.size() < K2) {
                v2 = V(gen);
                K2 = K(gen);
                //std::cout << "\n\tK2 = " << K2;
            }
        ...
        if (s.R[v1].rOrder.size() < K1 || mudou[v1]) continue;
        for (int pos1 = 0; pos1 < s.R[v1].rOrder.size() - K1 && !perturbou; pos1++){
            //current vector of transferred customers
            std::vector<int> currK1 (K1,0);
            int demK1 = 0;
            for (int k = 0; k < K1; k++) {
                currK1[k] = s.R[v1].rOrder[pos1 + k];
                demK1 += r.q[t][currK1[k]];                
            }
            
            if (s.R[v2].rOrder.size() < K2 || mudou[v2]) continue;
            if (v1 != v2){
                for (int pos2 = 0; pos2 < s.R[v2].rOrder.size() - K2 && !perturbou; pos2++){
                    //current vector of transferred customers
                    std::vector<int> currK2 (K2,0);
                    int demK2 = 0;
                    for (int k = 0; k < K2; k++) {
                        currK2[k] = s.R[v2].rOrder[pos2 + k];
                        demK2 += r.q[t][currK2[k]];                        
                    }
                    
                    //testando a viabilidade de cargas
                    int load1 = s.R[v1].rLoad - demK1 + demK2;
                    int load2 = s.R[v2].rLoad - demK2 + demK1;
                    if (load1 <= d.Q[v1] && load2 <= d.Q[v2]){
                        //std::cout << "\n\t\tcarga ok! v1 = " << v1 << ", v2 = " << v2;
                        std::deque<int> r1 (s.R[v1].rOrder);
                        std::deque<int> r2 (s.R[v2].rOrder);
                        
                        //apagando das rotas os strings
                        for (int k = 0; k < K1; k++){
                            r1.erase(r1.begin() + pos1);
                            int i = currK1[k];
                            r2.push_back(i);                            
                        ...
                        int ttt2 = checkRoutTTT(d,r2);
                        int ttt1 = checkRoutTTT(d,r1);
                        //std::cout << "\n\t\ttempo! ttt1 = " << ttt1 << ", ttt2 = " << ttt2;
                        if (ttt1 <= d.H && ttt2 <= d.H){
                            trocou = true;
                            mudou[v1] = true;
                            mudou[v2] = true;
                            perturbou = true;
                            
                            double oldV1Cost = s.R[v1].rCost;
                            double oldV2Cost = s.R[v2].rCost;
                            ...
                            /*
                            std::cout << green;
                            printRoute(d,s.R[v2],v2);
                            std::cout << normal;
                            */
                            s.R[v2] = newRoute(d.e[v2] + checkRoutCost(d,r2), ttt2, load2, r2, btR2);
                            r.z[t][v1] = boost::dynamic_bitset<> (btR2);
                            /*
                            std::cout << red;
                            printRoute(d,s.R[v2],v2);
                            std::cout << normal;
                            */
                            boost::dynamic_bitset<> btR1(d.n,0);
                            for (int pos = 0; pos < r1.size(); pos++) {
                                s.cpp[r1[pos]] = v1;
                                btR1.set(r1[pos]-1);                                
                            }
                            /*
                            std::cout << green;
                            printRoute(d,s.R[v1],v1);
                            std::cout << normal;
                            */
                            s.R[v1] = newRoute(d.e[v1] + checkRoutCost(d,r1), ttt1, load1, r1, btR1);
                            r.z[t][v1] = boost::dynamic_bitset<> (btR1);
                            /*
                            std::cout << blue;
                            printRoute(d,s.R[v1],v1);
                            std::cout << normal;
                            */
                            if (s.R[v1].rOrder.size() >= 1) updatePredSucc(d.n,s.R[v1].rOrder,s.ps);
                            if (s.R[v2].rOrder.size() >= 1) updatePredSucc(d.n,s.R[v2].rOrder,s.ps);
                            s.c = s.c - (oldV1Cost + oldV2Cost) + (s.R[v1].rCost + s.R[v2].rCost);                            
                        }                        
                    }
                }                
            }            
        }    
    }while (perturbou && totPert <= maxPertKK);
...
bool lexicographic(data d, std::vector<int> q, vrs &s){
    //std::cout << "\n\tlexicographic";
    int routesGreaterThanTwo = 0;
    for (int v = 1; v <= d.V; v++) if (s.R[v].rComp.count() >= 3) routesGreaterThanTwo++;

    if (routesGreaterThanTwo <= int(std::round(d.V/3))) return false;
    ...
    int totPert = 0;
    int maxPert = int(std::round(d.V/3));
    bool perturbou = false;
    
    //while (totPert <= maxPert){
    while (totPert <= maxPertLX){
        //sorteando os veiculos alterados
        int v = V(gen);
        while (mudou[v] || s.R[v].rComp.count() < 3) v = V(gen);
        
        boost::dynamic_bitset<> N (s.R[v].rComp);
        vrs s1 = cpyVRS(s);    
        int currV = v;
        /*
        std::cout << red;
        printRoute(d,s1.R[v],v);
        std::cout << normal;
        //*/
        s1.c -= s1.R[v].rCost;
        greedyRoute(d,q,v,N,s1);        
        
        //se a rota continuou viável em questão do tempo
        if (v == currV + 1 && N.none()) {            
            s = cpyVRS(s1);
            perturbou = true;
            /*
            std::cout << blue;
            printRoute(d,s1.R[currV],currV);
            std::cout << normal;            
            //*/
        }
        mudou[currV] = true;
...
bool randomized(data d, vrs &s){
    ...
    //while (totPert <= maxPert){
    while (totPert <= maxPertRD){
        //sorteando os veiculos alterados
        int v = V(gen);
        while (mudou[v] || s.R[v].rComp.count() < 3) v = V(gen);
        
        boost::dynamic_bitset<> N (s.R[v].rComp);
        int totCust = s.R[v].rComp.count();
        
        std::uniform_int_distribution<> C(1,totCust);
        
        std::deque<int> rota;
        while(N.any()){
            int pos = C(gen);
            while(N.test(pos-1)) pos = C(gen);
            int i = s.R[v].rOrder[pos];
            rota.push_back(i);
            N.reset(i-1);
        }
        
        int newTTT = checkRoutTTT(d,rota);
        if (newTTT <= d.H){
            double oldRouteCost = s.R[v].rCost;        
            s.R[v] = newRoute(d.e[v] + checkRoutCost(d,rota), newTTT, s.R[v].rLoad, rota, s.R[v].rComp);
            if (s.R[v].rOrder.size() > 0) updatePredSucc(d.n,s.R[v].rOrder,s.ps);
            s.c = s.c - oldRouteCost + s.R[v].rCost;
        }
        mudou[v] = true;
        totPert++;
    }
...
bool perturbations(data d, int t, int pT, routing &r, std::vector<vrs> &s){
    //std::cout << GREEN << "\n\tPERTURBATIONS" << normal;
    bool perturbou = false;

    routing rl = cpRout(r);
    std::vector<vrs> sl (s);
    
    //sorteas os veículos alterados e o numero deles que será modificado
    std::uniform_int_distribution<> V(1,d.V);

    //algumas perturbações são feitas para cada periodo        
    if (pT == 1){        
        //inverte a ordem de visitas de algumas rotas, so modifica s
        std::vector<bool> mudou (d.V + 1, false);
        int maxV = V(gen);
        int totv = 1;
        while (totv <= maxV){
            ...
            mudou[v] = true;
            totv++;
            if (sl[t].R[v].rComp.count() > 2){
                sl[t].R[v] = reverseRoute(d,sl[t].R[v],v);
                perturbou = true;
                updatePredSucc(d.n, sl[t].R[v].rOrder, sl[t].ps);
            }            
        }
    }else if (pT == 2) //redistribui clientes para algum veículo desocupado, modifica s,r
        if (rl.V[t] > 0 && rl.V[t] < d.V) perturbou = splitRoute (d,t,rl,sl[t]);
    else if (pT == 3) //modifica s,r
        perturbou = kkSwapRoutes(d, sl[t], rl, t);        
    else if (pT == 4) ///modifica s,r
        perturbou = lexicographic(d, r.q[t], sl[t]);
    else if (pT == 5) ///modifica s,r
        perturbou = randomized(d, sl[t]);
        
    if (perturbou){
        bool melhorou = interRouteOpt(d,rl,sl[t],t);
        r = cpRout(rl);        
        s = std::vector<vrs> (sl);
    }
    //std::cout << GREEN << "\n\tEND PERTURBATIONS" << normal;
    return perturbou;
}
...
double iils(data d){
    ...
    //registra o numero total de melhorias e também por cada vizinhanca
    std::vector<int> iPN(d.ntMax + 1,0);
    
    //enquanto ainda existe alguma perturbação capaz de melhorar a solução
    std::uniform_int_distribution<> PT(1,6);
    std::uniform_int_distribution<> T(1,d.T);
    std::uniform_int_distribution<> T2(2,d.T-1);
    ...
    do{
        ...
        //sinaliza a vizinhanca a ser pesquisada
        //tipo: (1) inversão de rota, (2) divisao de rota, (3) swap(k,k), (4) lexicografica, (5) randomica, (6) production
        std::vector<int> success (d.ntMax + 1,1);
        success[0] = 6;
        
        //enquanto ainda existe vizinhanca nao explorada
        while(success[0] > 0){
            //solucao vizinha (s')
            routing rl = cpRout(r);
            prodinv pl = cpProd(p);
            ...
            //seleciona a vizinhanca
            int pT = PT(gen);
            while(success[pT] == 0) pT = PT(gen);
            //std::cout << "\n\tpT = " << pT;
            
            //perturba PPP            
            bool pertPPP = false;                        
            //perturbacao do roteamento
            if (pT <= 5){                
                //entre 25% e 75% dos períodos são perturbados
                int maxShakenT = T(gen);
                while (maxShakenT < int(std::floor(d.T/4)) || maxShakenT > (d.T - int(std::floor(d.T/4)))) maxShakenT = T(gen); 
                
                if (maxShakenT == d.T) maxShakenT = T2(gen);            
                
                std::vector<bool> perturbado (d.T + 1, false);            
                int cntShakeT = 0;
                
                double oldFO3 = fol[3];
                while(cntShakeT <= maxShakenT){
                    int t = T(gen);
                    while (perturbado[t]) t = T(gen);                
                    bool perturbou = perturbations(d,t,pT,rl,vrpl);
                    perturbado[t] = true;
                    cntShakeT++;
                    prts[pT]++;
                    prts[0]++;
                ...
                //atualizando a solucao com as possiveis mudanças no roteamento
                fol[3] = 0;
                for (int t = 1; t <= d.T; t++) fol[3] += vrpl[t].c;
                fol[0] = (fol[0] - oldFO3 + fol[3]);
            }else//perturba PPP            
                pertPPP = modifyProdPlan(d,pl,vrpl, ppp);
                                                                    
			//atualizo delta(i,k,v,t), levo para o PPP e verifico se houve melhora otimizando
            updateDelta(d,vrpl,pl,ppp);                        
            ...
            if (fol[0] < fo[0] && vrpFeas && pppFeas){
				...
				//atualizando a vizinhanca que trouxe melhoras
				iPN[pT]++;
                iPN[0]++;
                if (fo[0] < fos[0]){
                    //atualizando a nova melhor solução
                ...
                if (fol[0] < (1 + alpha)*fo[0] && vrpFeas && pppFeas){
                    //atualizando a nova melhor solução
                    r = cpRout(rl);
                    p = cpProd(pl);
                    vrp = std::vector<vrs> (vrpl);
    ...
    std::cout << " >>";
    //quantidade de melhorias encontradas por cada vizinhanca
    for (int nt = 0; nt <= d.ntMax; nt++) std::cout << " " << iPN[nt];
    std::cout << " >>";
    //quantidade total de perturbacoes por cada vizinhanca
    for (int ls = 0; ls <= d.ntMax; ls++) std::cout << " " << prts[ls];    
    
 
    //std::cout << d.name << " >> fo = " << fos[0] << " = " << fos[1] << " + " << fos[2] << " + " << fos[3] << " >> t(s) = " << timeStar << std::endl;   
    solprp star;
    star.p = cpProd(ps);
    star.r = cpRout(rs);
...
int main(int argc, char *argv[]){        
    ...
    d.pathOut = "/home/varatan/Documentos/Dropbox/02-ufmg/02-doutorado/01-pesquisa/02-tese/03-implementacoes/02-prp/04-prp-v-indx-h/03-itg/01/";
